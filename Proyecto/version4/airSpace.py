...
class AirSpace:
    ...
    def add_navpoint(self, name, code, lat, lon):
        # Generar un ID único para el nuevo nodo
        if self.NavPoints:
            # Usar el máximo ID existente + 1
            max_id = max(point.code for point in self.NavPoints if isinstance(point.code, int))
            new_id = max_id + 1
        else:
            new_id = 1

        # Crear el nodo con ID numérico (manteniendo consistencia con nodos cargados)
        p = NavPoint(new_id, name, lat, lon)
        self.NavPoints.append(p)

        # IMPORTANTE: Actualizar el diccionario interno
        self._nav_points_dict[new_id] = p
        self._nav_points_dict[name] = p
        # También indexar por el código de texto si es diferente del nombre
        if code != name:
            self._nav_points_dict[code] = p
    ...
    def add_segment(self, origin_code, dest_code, distance):
        ...
        if origin and dest:
            origin.add_neighbor((dest, distance))  # Asegurar que es tupla
            dest.add_neighbor((origin, distance))  # Conexión bidireccional
            # Usar los IDs numéricos para el segmento
            self.NavSegments.append(NavSegment(origin.code, dest.code, distance))

    def remove_navpoint(self, code):
        point = self.get_navpoint_by_name_or_id(code)
        if not point:
            raise ValueError("Nodo no encontrado")

        # Eliminar conexiones desde otros nodos
        for other in self.NavPoints:
            other.neighbors = [(n, d) for n, d in other.neighbors if n != point]

        # Eliminar segmentos relacionados (usar el ID real del punto)
        self.NavSegments = [s for s in self.NavSegments
                            if s.origin != point.code and s.destination != point.code]

        # Eliminar del diccionario interno
        if point.code in self._nav_points_dict:
            del self._nav_points_dict[point.code]
        if point.name in self._nav_points_dict:
            del self._nav_points_dict[point.name]
    ...
    def remove_segment(self, origin_code, dest_code):
        ...
        origin.neighbors = [(n, d) for n, d in origin.neighbors if n != dest]
        dest.neighbors = [(n, d) for n, d in dest.neighbors if n != origin]

        self.NavSegments = [s for s in self.NavSegments if not (
                (s.origin == origin_code and s.destination == dest_code) or
                (s.origin == dest_code and s.destination == origin_code)
        )]

    def debug_nav_points(self):
        """Método para debuggear el estado de los nodos"""
        print("=== Estado actual de NavPoints ===")
        for point in self.NavPoints:
            print(f"ID: {point.code}, Nombre: {point.name}, Vecinos: {len(point.neighbors)}")

        print("\n=== Estado del diccionario interno ===")
        for key, point in self._nav_points_dict.items():
            print(f"Clave: {key} -> {point.name} (ID: {point.code})")
